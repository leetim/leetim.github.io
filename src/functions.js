...
function handleFiles(e) {
    ...
    var canvasWasm = document.getElementById('canvas-wasm');
    var canvasAsm = document.getElementById('canvas-asm');
    var canvasJS = document.getElementById('canvas-js');
    var canvasWidth = 600/1.5;
    var canvasHeight = 400/1.5;
    var ctx = canvasWasm.getContext('2d');
    var ctx2 = canvasAsm.getContext('2d');
    var ctx3 = canvasJS.getContext('2d');
    var url = URL.createObjectURL(e.target.files[0]);
    var img = new Image();
    img.onload = function () {
        var scaleFactor = Math.min((canvasWidth / img.width), (canvasHeight / img.height));
        canvasWasm.width = img.width * scaleFactor;
        canvasWasm.height = img.height * scaleFactor;
        canvasAsm.width = img.width * scaleFactor;
        canvasAsm.height = img.height * scaleFactor;
        canvasJS.width = img.width * scaleFactor;
        canvasJS.height = img.height * scaleFactor;
...
function getInput() {
    var canvasWasm = document.getElementById('canvas-wasm');
    var ctx = canvasWasm.getContext('2d');
    var imgData = ctx.getImageData(0, 0, canvasWasm.width, canvasWasm.height);
    return imgData;
}
...
function detectFaceWasm() {
    ...
    (function () {
        ...
        show_image(img_color, "canvas-wasm");
...
function download() {
    var dt = canvasAsm.toDataURL('image/jpeg');
    this.href = dt;
};
